    ...
    fun kafkaProducerFactory(): ProducerFactory<String, Any> {
        val configProps =
            mapOf<String, Any>(
                ...
                ProducerConfig.ENABLE_IDEMPOTENCE_CONFIG to true, // 멱등성 설정
                ProducerConfig.ACKS_CONFIG to "all", // 모든 래풀라카의 확인을 받는 것은 tradeoff 사항
                ProducerConfig.RETRIES_CONFIG to 3, // 재시도 횟수 설정
                ProducerConfig.MAX_IN_FLIGHT_REQUESTS_PER_CONNECTION to 5, // 동시 전송 가능한 최대 요청 수 (5이하는 순서 보장 가능)
