...
class OpenApiConfig(
    ...
    @Bean
    @Profile("!Prod")
    fun boardRecommentApi(): GroupedOpenApi =
        createGroupedOpenApi(
            groupName = "게시글 대댓글 관련 api",
            paths = arrayOf("/api/board-recomment/**"),
        )
    ...
    fun loginApi(): GroupedOpenApi =
        GroupedOpenApi
            .builder()
            .group("로그인 관련 api")
            .pathsToMatch("/api/member/login", "/api/member/logout")
            .addOpenApiCustomizer(loginEndpointCustomizer())
            .addOpenApiCustomizer(logoutEndpointCustomizer())
    ...
    @Bean
    fun logoutEndpointCustomizer(): OpenApiCustomizer = springSecurityLogoutEndpointCustomizer(applicationContext)
            ...
            for (filterChain in filterChainProxy.filterChains) {
                ...
                optionalFilter?.let { usernamePasswordAuthenticationFilter ->
                    ...
                    apiResponses.addApiResponse(
                        ...
                        ApiResponse()
                            .description(HttpStatus.UNAUTHORIZED.reasonPhrase)
                            .content(
                                Content().addMediaType(
                                    org.springframework.http.MediaType.APPLICATION_JSON_VALUE,
    ...
    /**
     * Swagger 문서에서 로그아웃 엔드포인트를 표시하기 위한 커스터마이저
     * Spring Security의 LogoutFilter를 감지하여
     * 해당 엔드포인트의 API 문서를 동적으로 생성합니다.
     */
    fun springSecurityLogoutEndpointCustomizer(applicationContext: ApplicationContext): OpenApiCustomizer {
        val filterChainProxy =
            applicationContext.getBean(
                AbstractSecurityWebApplicationInitializer.DEFAULT_FILTER_NAME,
                FilterChainProxy::class.java,
        ...
        return OpenApiCustomizer { openAPI ->
            ...
            val apiResponse = ApiResponses()
            apiResponse.addApiResponse(
                HttpStatus.OK.value().toString(),
                ApiResponse()
                    .description("로그아웃 성공")
                    .content(
                        Content().addMediaType(
                            org.springframework.http.MediaType.APPLICATION_JSON_VALUE,
                            MediaType().example("{\"message\":\"로그아웃 성공\",\"status\":200}"),
                        ),
                    ),
            )
            apiResponse.addApiResponse(
            ...
            operation.description("사용자 로그아웃을 처리합니다. 인증 토큰이 redis에서 삭제됩니다.")

            val pathItem = PathItem().post(operation)
            openAPI.paths.addPathItem("/api/member/logout", pathItem)
        }
    }
}