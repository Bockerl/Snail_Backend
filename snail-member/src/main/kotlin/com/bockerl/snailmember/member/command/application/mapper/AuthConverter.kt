...
class AuthConverter {
    // 회원가입 요청 vo to dto
    fun emailRequestVOToDTO(requestVO: EmailRequestVO) = EmailRequestDTO(
    ...
    // 이메일 인증 코드 vo to dto
    fun emailVerifyRequestVOToDTO(requestVO: EmailVerifyRequestVO) = EmailVerifyRequestDTO(
    ...
    fun phoneRequestVOToDTO(requestVO: PhoneRequestVO): PhoneRequestDTO = PhoneRequestDTO(
        phoneNumber = requestVO.phoneNumber ?: throw CommonException(ErrorCode.INVALID_INPUT_VALUE),
        redisId = requestVO.redisId ?: throw CommonException(ErrorCode.INVALID_INPUT_VALUE),
    )

    // 휴대폰 인증 코드 vo to dto
    fun phoneVerifyRequestVOToDTO(requestVO: PhoneVerifyRequestVO) = PhoneVerifyRequestDTO(
        verificationCode = requestVO.verificationCode ?: throw CommonException(ErrorCode.INVALID_INPUT_VALUE),
        redisId = requestVO.redisId ?: throw CommonException(ErrorCode.INVALID_INPUT_VALUE),
    )

    // 비밀번호 vo to dto
    fun passwordRequestVOToDTO(requestVO: PasswordRequestVO): PasswordRequestDTO = PasswordRequestDTO(
        password = requestVO.password ?: throw CommonException(ErrorCode.INVALID_INPUT_VALUE),
        redisId = requestVO.redisId ?: throw CommonException(ErrorCode.INVALID_INPUT_VALUE),
    )

    // 활동지역 vo to dto
    fun activityAreaRequestVOToDTO(requestVO: ActivityAreaRequestVO): ActivityAreaRequestDTO = ActivityAreaRequestDTO(
        redisId = requestVO.redisId ?: throw CommonException(ErrorCode.INVALID_INPUT_VALUE),
